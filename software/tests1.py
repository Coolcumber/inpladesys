...
for model in models:
    m = model()
    iterations = 1
    if isinstance(m, DummyStochasticAuthorDiarizer) or \
            (isinstance(m, SimpleFixedAuthorDiarizer) and dataset_index == 2):
        iterations = 50

    for i in range(iterations):
        print('Iteration {}/{}: {}'.format(i+1, iterations, model.name))
        m.fit(dataset.documents, dataset.segmentations)
        ...
        # calculate average on dataset and add to new array
        modelsToScores[model.name] /= len(dataset.documents)
        averagedModelsToScores[model.name] += modelsToScores[model.name]
        modelsToScores[model.name] = np.array([0, 0, 0], dtype=float)

    # calculate average on iterations
    averagedModelsToScores[model.name] /= iterations
    print()

print('\nFINAL RESULTS:')
for model in models:
    print(model.name)
    print(averagedModelsToScores[model.name])
    print()
