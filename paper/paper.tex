...
The most of described approaches combine supervised and usupervised methods and operate on the level of longer text segments or sentences. Since the style change in our tasks can occur even between two tokens in a same sentence, we wanted our model should be able to work on the token level. We were also inspired by \citet{brooke-2013} who said that a more radical approach would not separate described tasks in segmentation and clustering steps, but rather build an authorial segments that would also form good clusters. Instead of clustering tokens directly, we decided to cluster their vectorized stylistic contexts because they obviously contain more valuable sylistic information than tokens alone. 

\section{Author diarization and intrinsic plagiarism detetion} \label{sec:author-diraization}
\section{Author diarization and intrinsic plagiarism detection}
...
>>>>>>> fafd01a87e13338c26cd96ec9e2b122fe17b274e
...
\paragraph{Tokenization.} As a preprocessing step, we tokenized each document $D$ with \texttt{nltk}\footnote{\texttt{http://www.nltk.org}} toolkit, to obtain a set of tokens $T_D$. We also performed Part of Speech tagging with the same tool, to speed up a basic feature extraction which we describe below. We didn't use other preprocessing techniques such as lemmatization, stemming and stop word removal because they would take away a lot of stylometric data from text \cite{stamatatos-2009a}. The final output from tokenization step was a finite sequence $\{(t_i, o_i, l(t_i), POS_i)\}_{i=1}^n$ where $o_i$ is the offset of token $t_i$, $l(t_i)$ its length in characters and $POS_i$ its POS tag.

\paragraph{Basic feature extraction.} 
We defined the stylistic context of a token $t_i$ as a set of tokens [SYMBOL] where [SYMBOL] is a context size. From each context we extracted the most used stylometric features which we found in previous work of ():
\begin{itemize}
	\item \textit{Character tri-grams}. Frequencies of \textit{n}-grams on character level have been very useful in quantifying the writing style \citep{stamatatos-2009b}. They are able to capture lexical and contextual information, use of punctuation and errors which can be an author's  "fingerprints". This feature is also tolerant to noise. Based on work by \cite{stamatatos-2009a} and \citet{rahman-2015}, we choose $n=3$. Maximal dimension of this feature vector was set to 200.
	\item \textit{Stop words}. According to \cite{stamatatos-2009a}, sometimes also called \textit{function words}, these are the most common used topic-independent words in text, such as articles, prepositions, pronouns and others. Authors use them mainly unconciously and are found to be one of the most discriminative in authorship attribution since they represent pure stylistic choices of authors' \cite{burrows-1987,argamon-2005}.
	\item \textit{Special characters}.
	\item \textit{POS tag counts}.
	\item \textit{Average token length}.
	\item \textit{Bag of Words}.
	\item \textit{Type-token ratio}.
\end{itemize}
