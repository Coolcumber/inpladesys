...
% TODO: replace model A with Model A, for B as well
% TODO: replace $n$-gram and \emph{n}-gram with n-gram, as it is common in literature
% TODO: hange bi-grams to bigrams and tri-grams to trigrams
% TODO: suggestion: remove \texttt-s 
% TODO: change \texit-s ti \emph-s
% TODO: explain BFE training in The proposed approach
% TODO: consistency: use only "training set" and "test set" (not dataset, not train) (https://en.wikipedia.org/wiki/Test_set)
% TODO: convert Related work to present
...
In the third baseline, \textit{simple fixed baseline}, we used default k-means clustering of vectorized document's sentences. Average token length, sentence length, POS tag counts and bag of maximum 100 words (with stop words) served as a features. If the number of authors was unknown, it was randomly generated from $(0, 0.0003\times{l(d\in{D})})$ and increased by $1$ if the result was $0$.
...
We have presented some ideas that are not completely explored but, based on the data we have available, seem to give comparable, or even better results than the other two approaches that tired to tackle the same problem. There is a lot of room for improvement of our approach, as well as in exploring other new ideas.

For further work, we could include more basic features, try other clustering  algorithms and use more systematic approach for choosing (unknown) hyperparameters. Our transformation could be tested on other authorship analysis tasks.
