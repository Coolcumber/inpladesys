...
We have evaluated our models using micro-averaged and macro-averaged precision, recall and $F_1$-score for intrinsic plagiarism detection and the analogous BCubed scores for author diarization, as defined in subsection \ref{subsec:evaluation-measures}.

The results of evaluation for the intrinsic plagiarism problem (task \emph{a}) are shown in table \ref{tab:results-task-a}. For author diarization with known (task \emph{b}) and unknown (task \emph{c}) numbers of authors, the results are shown in table \ref{tab:results-task-b-c}.
